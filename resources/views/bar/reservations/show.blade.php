         ...
         <div class="row">
            <div class="col-lg-6">
               <div class="card">
                  <!-- /.card-header -->
                  <div class="card-body">
                     <form class="form-horizontal form-label-left" method="POST" action="{{ url('user/reservations/update') }}/{{$records->id}}"
                        enctype="multipart/form-data" novalidate>
                        @csrf
                        <div class="form-group">
                            <label>Table *</label>
                            <select class="form-control" name="table_id">
                                @foreach(get_list('restaurant_tables', array('restaurant_id'=>Auth::user()->restaurant_id)) as $row)
                                  <option value="{{ $row->id }}" 
