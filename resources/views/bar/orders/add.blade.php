...
<div class="col-lg-6">
    ...
    <!-- /.card-header -->
                <div class="card-body">
                    <form class="form-horizontal form-label-left" method="POST" action="{{ url('user/orders/add') }}"
     ...
     <tr>
      ...
      <td>
        <select x-model="field.txt1" type="text" name="item_id[]" class="form-control">
            ...
            @foreach(getListProductsPerUser(Auth::user()->restaurant_id) as $row)
       ...
       <td colspan="4" class="text-right"><button type="button" class="btn btn-default" @click="addNewField()">+ Add Row</button></td>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="waiter_id">
                                ...
                                @foreach(get_list('users', array('restaurant_id' => Auth::user()->restaurant_id, 'role'=>'waiter')) as $row)
                                    <option value="{{ $row->id }}">{{ $row->name }}</option>
                                @endforeach
                            </select>
                        </div>
