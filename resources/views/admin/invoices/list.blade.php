...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-7">
            <h1>Invoices <span><small>List</small></span></h1>
            <!--<a class="btn btn-primary mt-3" href="{{ url('admin/invoices/add') }}" >Add Sale</a>-->
        ...
        <div class="col-sm-5">
            ...
            <!--          <option @if (isset($search_type) && $search_type== 'date') selected="selected" @endif value="date">Search By Invoice</option>-->
        ...
        <div class="card-body">
          ...
          <table class="table mb-5" width="100%">
                  ...
                  <tbody>
                      ...
                      <tr>
                          <td>POS#{{ $data->id }}</td>
                          <td><?= get_name_by_id('users','name',array('id'=>$data->user_id, 'role'=>'user', 'restaurant_id'=>0)) ?? "<span style='color:orange'>No User Found</span>"; ?></td>
                          <td class="text-center">
                          <?php if($data->payment_status == 'paid'): ?>
                          <span class="badge badge-success">Paid</span>
                          <?php else: ?> 
                          <a href="{{ url('admin/orders/toPaid') }}/{{ $data->id }}" ><span class="badge badge-danger">Due</span></a>
                          <?php endif; ?>
                          </td>
                          <td>{{ $data->created_at }}</td>
                          <td class="text-center">${{ $data->grand_total }}</td>
                          <td class="project-actions text-right">
                            <a class="btn btn-primary btn-sm" href="{{ url('admin/invoices/invoice') }}/{{$data->id}}">
